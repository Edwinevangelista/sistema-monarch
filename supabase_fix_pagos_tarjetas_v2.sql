...
-- Fix 1: hacer "tarjeta" nullable — el nuevo flujo usa deuda_id en vez del nombre
alter table public.pagos_tarjetas
  alter column tarjeta drop not null;

-- Fix 2: actualizar el RPC para pasar el nombre de tarjeta y que funcione con ambos esquemas
...
begin
  ...
  select balance, nombre
    into v_saldo_cuenta, v_cuenta_nombre
  ...
  select saldo, cuenta
    into v_deuda_actual, v_tarjeta_nombre
  from public.deudas
  ...
  values (
    p_user_id,
    p_tarjeta_id,
    coalesce(v_tarjeta_nombre, 'Crédito'),
    p_cuenta_id,
    p_monto,
    p_monto,
    0,
...
revoke all on function public.pagar_tarjeta(
  uuid, uuid, uuid, numeric, text, date, text, text, numeric, date
) from public;

grant execute on function public.pagar_tarjeta(
  uuid, uuid, uuid, numeric, text, date, text, text, numeric, date
) to authenticated;
