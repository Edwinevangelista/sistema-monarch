...
function calculateMonthlyPlan(debts, capacity) {
  ...
  // Límite de 120 meses (10 años) como máximo realista
  while (remainingDebts.length > 0 && month < 120) {
    ...
    // Pagar deudas según estrategia (prioridad a la primera)
    for (let i = 0; i < remainingDebts.length; i++) {
      const debt = remainingDebts[i];
      
      // Primera deuda recibe el extra, las demás solo pago mínimo
      const payment = i === 0 
...
function generateRecommendations(debts, summary, capacity, kpis) {
  ...
  if (summary.monthsToPayoff > 36) {
    recommendations.push({
      ...
      priority: 'high',
      action: 'Busca formas de aumentar el pago mensual en $50-100'
    });
  }
  ...
  if (extraPayment > 0) {
    recommendations.push({
      ...
      priority: 'medium',
      action: `Reducirías ${Math.round((summary.monthsToPayoff * 0.3))} meses del plan`
    });
  }
...
export default function DebtPlannerModal({ deudas = [], kpis = {}, onClose }) {
  ...
  const generatePlan = () => {
    try {
      ...
      console.log('Generando plan con:', { selected, kpis, strategy });
