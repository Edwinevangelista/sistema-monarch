...
export default function SubscriptionOptimizerModal({ suscripciones, kpis, onClose }) {
  // CORRECCIÓN DE DATOS: 
  // Aseguramos que suscripciones sea un array y kpis un objeto para evitar errores de "Cannot read property of undefined"
  const safeSuscripciones = Array.isArray(suscripciones) ? suscripciones : [];
  const safeKpis = typeof kpis === 'object' && kpis !== null ? kpis : {};

  // Filtrar suscripciones activas (Protegido contra suscripciones sin propiedad 'estado')
  const activeSubs = safeSuscripciones.filter(s => s && s.estado === 'Activo');

  // Calcular total mensual (Protegido contra 'costo' nulo o texto)
  const totalMensual = activeSubs.reduce((sum, sub) => {
    // Convertir costo a número, si falla usar 0
  ...
  // Obtener ingresos con valor por defecto
  const totalIngresos = Number(safeKpis.totalIngresos) || 0;
  
  // CORRECCIÓN MATEMÁTICA: 
  // Prevenir división por cero. Si ingresos es 0, porcentaje es 0.
  const porcentajeIngresos = totalIngresos > 0 
    ? (totalMensual / totalIngresos) * 100 
    : 0;

  // Detectar duplicados (Protegido contra suscripciones sin categoría)
  const categorias = {};
  activeSubs.forEach(sub => {
    // Si categoría no existe, usar 'Otros'
  ...
  const caras = activeSubs.filter(sub => {
    ...
    // Solo chequear si hay ingresos válidos
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/70 flex items-center justify-center p-0 sm:p-4">
      <div className="relative flex min-h-screen items-center justify-center w-full">
        <div className="bg-gray-800 rounded-2xl w-full max-w-4xl m-4 shadow-2xl flex flex-col max-h-[85vh]">
          
          {/* Header */}
          <div className="flex-none bg-gradient-to-r from-indigo-600 to-purple-600 p-6 rounded-t-2xl flex justify-between items-center">
            <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-orange-500/20 border border-orange-500/30 rounded-xl p-4">
                ...
                <div className="text-orange-200 text-xs mt-1">
                  {totalIngresos > 0 
                    ? `${porcentajeIngresos.toFixed(1)}% de tus ingresos` 
            ...
            {duplicados.length === 0 && caras.length === 0 && (
              <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-6 text-center">
                ...
                <div className="text-green-100 text-sm">
                  ...
                  {totalIngresos > 0 && (
                    <span> Tus suscripciones representan {porcentajeIngresos.toFixed(1)}% de tus ingresos.</span>
                  )}
                </div>
              </div>
            )}
