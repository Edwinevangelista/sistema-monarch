 ...
 * - Mostrar listas
 * - Emitir eventos (ver / pagar / eliminar)
...
export default function ListaGastosCompleta({
  ...
  const renderCard = (item, type) => {
    // Estado pagado SOLO donde aplica
    const isPaid =
      type === ITEM_TYPES.DEUDA
        ? deudaPagadaEsteMes?.(item.id)
        : type === ITEM_TYPES.FIJO
        ? item.estado === 'Pagado'
        : false // variables y suscripciones no se marcan aquí como pagadas
    ...
    return (
      <div
        ...
        className="bg-gray-900 border border-gray-700 rounded-xl p-4 hover:border-gray-500 transition cursor-pointer"
      >
        {/* HEADER */}
        ...
        {/* ACCIONES */}
        <div
          className="flex gap-2 mt-4 pt-3 border-t border-gray-800"
        ...
        >
          ...
          {/* Acción principal SOLO si aplica */}
    ...
    <div className="space-y-6">
      ...
      {suscripciones.length > 0 && (
        <section>
          ...
          <div className="space-y-3">
            {suscripciones.map((s) =>
              renderCard(s, ITEM_TYPES.SUSCRIPCION)
            )}
          </div>
        </section>
      )}
      ...
      {gastosVariables.length > 0 && (
        <section>
          ...
          <div className="space-y-3">
            {gastosVariables.map((v) =>
              renderCard(v, ITEM_TYPES.VARIABLE)
            )}
          </div>
        </section>
      )}
