...
export default function ListaGastosCompleta({
  ...
  const renderCard = (item, type) => {
    ...
    // ===========================================
    // LÓGICA DE ESTADO PARA SUSCRIPCIONES
    // ===========================================
    
    let isPaid = false;
    let estadoSuscripcion = null; // 'pagada' | 'pendiente' | 'vence_hoy' | 'vencida'
    
    // 1. Deudas
    if (type === ITEM_TYPES.DEUDA && deudaPagadaEsteMes?.(item.id)) {
      isPaid = true;
    }
    
    // 2. Gastos Fijos
    if (type === ITEM_TYPES.FIJO && item.estado === 'Pagado') {
      isPaid = true;
    }
    
    // 3. Suscripciones (LÓGICA PROFESIONAL)
    if (type === ITEM_TYPES.SUSCRIPCION) {
      ...
      if (proximoPago < hoy) {
        ...
        isPaid = false;
      } else if (proximoPago.getTime() === hoy.getTime()) {
        // Fecha exacta hoy → VENCE HOY
        estadoSuscripcion = 'vence_hoy';
        isPaid = false;
      } else if (
        proximoPago.getFullYear() === hoy.getFullYear() &&
        proximoPago.getMonth() === hoy.getMonth()
      ) {
        // Fecha futura dentro del mismo mes → PENDIENTE
        estadoSuscripcion = 'pendiente';
        isPaid = false;
      } else {
        // Fecha en mes/año siguiente → PAGADA
        estadoSuscripcion = 'pagada';
        isPaid = true;
      }
    }
          ...
          <div className="flex items-start gap-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                {type === ITEM_TYPES.SUSCRIPCION && estadoSuscripcion === 'vence_hoy' && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-orange-500/20 text-orange-400 border border-orange-500/30 flex items-center gap-1 shrink-0 animate-pulse">
                    ⏰ VENCE HOY
                  </span>
                )}
                
                {type === ITEM_TYPES.SUSCRIPCION && estadoSuscripcion === 'vencida' && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-red-500/20 text-red-400 border border-red-500/30 flex items-center gap-1 shrink-0">
                    ⚠️ VENCIDA
                  </span>
                )}
                
                {/* BADGES PARA GASTOS FIJOS PAGADOS */}
                {type === ITEM_TYPES.FIJO && isPaid && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 flex items-center gap-1 shrink-0">
